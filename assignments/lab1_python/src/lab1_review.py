...
def every_third(message: str) -> str:
    ...
    '''
    return_str = message[1]
    for i in range(4, len(message), 3):
        return_str += message[i]
...
def half_upper_case(message: str) -> str:
    ...
    for j in range(len(message)//2, len(message)):
        new_message += message[j].lower()
...
def lcm(x: int, y: int) -> int:
    ...
    testval = m
    while True:
        if testval % x == 0 and testval % y == 0:
            return testval
...
def decrement_string(mystr: str) -> str:
    ...
    '''
    return "".join([chr(ord(i) - 1) for i in mystr])
...
class BlueBall:
    ...
    def find_ball_pix(self, show=False) -> np.array:
        ...
        '''
        ball_pix = np.zeros(self.img.shape[:2], dtype="uint8")
    ...
    def centroid_of_ball(self, show=False) -> tuple:
        ...
        center_xy = np.where(self.find_ball_pix() == 1)
        center_xy = average(center_xy[1]), average(center_xy[0])
