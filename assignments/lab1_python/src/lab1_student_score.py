...
from lab1_review import BlueBall
...
class TestImageOps(unittest.TestCase):
    ...
    def setUp(self):
        self.path_to_folder = '..'  # If run in VSCode folder, this path may need to be: 'labs_2021/lab1_python'
        self.data_folder = 'data'
        self.gt_folder = 'gt'
        self.ball = BlueBall(os.path.join(self.path_to_folder,self.data_folder,'ball.jpg'))
        self.assertTrue( self.ball.init_succeed )

    def test_canny_edge(self):
        gt = cv.imread(os.path.join(self.path_to_folder,self.gt_folder,'ball_canny.png'), cv.IMREAD_GRAYSCALE )
        npt.assert_equal( self.ball.canny_edge(100,200,True)>0, gt>0)

    def test_fine_ball_pix(self):
        gt = cv.imread(os.path.join(self.path_to_folder,self.gt_folder,'ball_mask.png'), cv.IMREAD_GRAYSCALE )
        npt.assert_equal( self.ball.find_ball_pix(True)>0, gt>0)

    def test_centroid_of_ball(self):
        cengt = np.array( list((183, 162)) )
        cen = np.array( list( self.ball.centroid_of_ball(True)) )
        npt.assert_allclose(cen, cengt, atol=2)
